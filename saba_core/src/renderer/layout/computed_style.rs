...
use crate::renderer::dom::{element::ElementKind, node::Node};
...
impl ComputedStyle {
    ...
    pub fn defaulting(
        ...
        if let Some(parent_style) = parent_style {
            if self.background_color.is_none()
                && parent_style.background_color() != Color::white()
            {
            ...
            if self.font_size.is_none()
                && parent_style.font_size() != FontSize::Medium
            {
    ...
    pub fn set_display(&mut self, d: DisplayType) {
        self.display = Some(d)
    }

    pub fn font_size(&self) -> FontSize {
        self.font_size.expect("failed to access CSS property: font-size")
    }
...
impl FontSize {
    fn default(node: &Rc<RefCell<Node>>) -> Self {
        match &node.borrow().kind() {
            NodeKind::Element(elem) => match elem.kind() {
